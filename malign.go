...
// Code below copy from https://github.com/mdempsky/maligned
// with modification for reporting suggested fixes.

func malign(origStruct *types.Struct) result {
	wordSize := int64(8)
	maxAlign := int64(8)
	...
	optStruct := optimalSize(origStruct, &s)
	return result{
		oldSize:         s.Sizeof(origStruct),
		newSize:         s.Sizeof(optStruct),
		suggestedStruct: optStruct,
	}
}

func optimalSize(str *types.Struct, sizes *gcSizes) *types.Struct {
	nf := str.NumFields()
